...
public class Master : MonoBehaviour {
    ...
    void CreateScene () {
        List<DifferentShapes> allShapes = new List<DifferentShapes> (FindObjectsOfType<DifferentShapes> ());
        allShapes.Sort ((a, b) => a.operation.CompareTo (b.operation));

        List<DifferentShapes> orderedShapes = new List<DifferentShapes> ();
        ...
        for (int i = 0; i < allShapes.Count; i++) {
            ...
            if (allShapes[i].transform.parent == null) {
                ...
                for (int j = 0; j < parentShape.childCount; j++) {
                    if (parentShape.GetChild (j).GetComponent<DifferentShapes> () != null) {
                        orderedShapes.Add (parentShape.GetChild (j).GetComponent<DifferentShapes> ());
